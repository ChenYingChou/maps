...
G.setOnLoadCallback(function() {
	...
	// 模版源碼 2010/05/01 http://github.com/nje/jquery-tmpl, 以 google-compiler 編譯如下:( 1791 bytes)
	(function(b){var j=b.fn.domManip,k=/^[^<]*(<[\w\W]+>)[^>]*$/;b.fn.extend({render:function(a,e){return this.map(function(h,c){return b.render(c,a,e)})},domManip:function(a){if(a.length>1&&a[0].nodeType)arguments[0]=[b.makeArray(a)];if(a.length>=2&&typeof a[0]==="string"&&typeof a[1]!=="string")arguments[0]=[b.render(a[0],a[1],a[2])];return j.apply(this,arguments)}});b.extend({render:function(a,e,h){var c,d;if(typeof a==="string"){c=b.templates[a];if(!c)if(k.test(a))c=b.tmpl(a);else d=b(a).get(0)}else if(a instanceof
		b)d=a.get(0);else if(a.nodeType)d=a;if(!c&&d){a=b.data(d);c=a.tmpl||(a.tmpl=b.tmpl(d.innerHTML))}var g={data:e,index:0,dataItem:e,options:h||{}};return b.isArray(e)?b.map(e,function(f,i){g.index=i;g.dataItem=f;return c.call(f,b,g)}):c.call(e,b,g)},templates:{},tmplcmd:{each:{_default:[null,"$i"],prefix:"jQuery.each($1,function($2){with(this){",suffix:"}});"},"if":{prefix:"if($1){",suffix:"}"},"else":{prefix:"}else{"},html:{prefix:"_.push(typeof ($1)==='function'?($1).call(this):$1);"},"=":{_default:["this"],
		prefix:"_.push($.encode(typeof ($1)==='function'?($1).call(this):$1));"}},encode:function(a){return a!=null?document.createTextNode(a.toString()).nodeValue:""},tmpl:function(a,e,h,c){a=new Function("jQuery","$context","var $=jQuery,$data=$context.dataItem,$i=$context.index,_=[];_.data=$data;_.index=$i;with($data){_.push('"+a.replace(/[\r\t\n]/g," ").replace(/\${([^}]*)}/g,"{{= $1}}").replace(/{{(\/?)(\w+|.)(?:\((.*?)\))?(?: (.*?))?}}/g,function(d,g,f,i,l){d=b.tmplcmd[f];if(!d)throw"Template not found: "+
		f;f=d._default;return"');"+d[g?"suffix":"prefix"].split("$1").join(l||(f?f[0]:"")).split("$2").join(i||(f?f[1]:""))+"_.push('"})+"');};return $(_.join('')).get();");return e?a.call(this,b,{data:null,dataItem:e,index:h,options:c}):a}})})(jQuery);
